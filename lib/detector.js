...
const AI_CRAWLERS = [
  ...
  // NOTE: Google-CloudVertexBot is listed below under 'Google CloudVertex'
  // NOTE: Googlebot-AI has no verified documented UA — removed
  {
    name: 'SearchGPT',
...
// (`node lib/detector.js`), not when required as a module. Previously ran
// unconditionally on every require(), printing test output to stdout for
// every consumer (api/index.js, combined-detector.js, generation scripts,
// etc.) — harmless but noisy, and broke scripts that capture stdout
// (Session 7: scripts/generate-worker-detector.js).
