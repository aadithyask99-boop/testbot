...
async function selectVariant(winner, pageSignals) {
  let variants = winner.variants || [];
  if (variants.length === 0) {
    // Should never happen — isEligible() requires non-empty variants.
    return { variant: null, variantId: null, method: 'none' };
  }

  // Session 10 Batch 6: AI Recommendations approval gate.
  // Once a campaign has ANY approved recommendation, only approved
  // variants are eligible to serve. Unreviewed variants in the bank
  // become a draft pool, invisible to the live auction. A campaign
  // with zero approvals yet falls back to its full raw variant bank
  // (so a brand-new campaign isn't dead on arrival).
  try {
    const approved = await kvGet(`approved:${winner.id}`);
    if (approved && approved.length > 0) {
      const approvedIds = new Set(approved.map(a => a.variantId));
      const approvedVariants = variants.filter(v => approvedIds.has(v.id));
      if (approvedVariants.length > 0) variants = approvedVariants;
    }
  } catch (e) { /* non-fatal — fall back to full bank on KV error */ }
