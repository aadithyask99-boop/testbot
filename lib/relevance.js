...
const { kvGet, kvSetWithTTL } = require('./kv');
...
const TAXONOMY = {
  ...
  tech: {
    tier1: ['vpn', 'saas', 'kubernetes', 'docker', 'react', 'python', 'github',
            'developer tools', 'cloud computing'],
...
async function haikuFilterRelevant(candidates, pageSignals) {
  ...
  const candidateLines = candidates.map((c, i) => {
    ...
    const copy = (c.text || '(no copy)').replace(/\s+/g, ' ').trim();
...
// ============================================================
// LAYER 4: Per-campaign relevance scoring (keyword)
// ============================================================
...
async function runAuctionForCategory(category, pageSignals, meta = {}) {
  ...
  if (keywordSurvivors.length === 0) {
    return {
      ...
      candidateScores: keywordScored.map(c => ({
        id: c.id, advertiser: c.advertiser, relevanceScore: c.relevanceScore,
      })),
      ...meta,
    };
  }
  ...
  if (!winner) {
    return { winner: null, category, reason: 'all_over_budget', ...meta, haikuUsed };
  }
...
async function runMatch(pageSignals) {
  ...
  try {
    ...
    if (cached && cached.category) {
      return await runAuctionForCategory(cached.category, pageSignals, {
    ...
    // Cache read failure is non-fatal — fall through to classify
  ...
  if (publisherCategory && CATEGORIES.includes(publisherCategory)) {
    // Cache the publisher-declared category so future hits skip the cascade
    try {
      await kvSetWithTTL(cacheKey, {
        category: publisherCategory,
        method: 'publisher_tag',
        classifiedAt: Date.now(),
      }, CACHE_TTL_SECONDS);
    } catch (e) { /* non-fatal */ }
    return await runAuctionForCategory(publisherCategory, pageSignals, {
      method: 'publisher_tag',
      cached: false,
    });
  }
  ...
  if (kw.score < KEYWORD_CONFIDENT_SCORE || kw.category === 'other') {
    ...
    if (haikuResult) {
      ...
      // Haiku failed → stick with keyword result (no logged downgrade in response)
      method = 'keyword_haiku_fallback';
    }
  }

  // Cache the classification (NOT the winner — winners change as campaigns do)
  try {
    await kvSetWithTTL(cacheKey, {
  ...
  // ─── LAYER 4 + 5 ───────────────────────────────────────────
  return await runAuctionForCategory(finalCategory, pageSignals, {
    method,
    cached: false,
    keywordScore: kw.score,
    keywordAllScores: kw.allScores,
  });
}
