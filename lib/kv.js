...
async function kvRequest(command, debug = false) {
  if (!BASE_URL || !TOKEN) {
  ...
  if (debug) return { status: response.status, data };
  return data.result;
}

// TEMPORARY DEBUG: raw fetch to Upstash for a SET + GET, returning full
// response bodies and status codes. SAFE TO REMOVE after the incident.
async function kvDebugRoundtrip() {
  const key = '__debug_kv_selftest__';
  const value = encodeURIComponent(JSON.stringify({ ping: Date.now() }));
  const setResp = await kvRequest(['SET', encodeURIComponent(key), value], true);
  const getResp = await kvRequest(['GET', encodeURIComponent(key)], true);
  return { setResp, getResp, baseUrlSet: !!BASE_URL, tokenSet: !!TOKEN, baseUrlPrefix: (BASE_URL || '').slice(0, 30) };
}

// TEMPORARY DEBUG: return the raw value from Upstash with no parsing,
// plus its JS type, length, and the first/last 40 chars. Used by
// /admin/debug-kv during the Session 5 incident.
// SAFE TO REMOVE after the incident is resolved.
async function kvDebugRaw(key) {
  try {
    const result = await kvRequest(['GET', encodeURIComponent(key)]);
    if (result === null || result === undefined) return { value: null, type: typeof result };
    const str = typeof result === 'string' ? result : JSON.stringify(result);
    return {
      type: typeof result,
      length: str.length,
      head: str.slice(0, 60),
      tail: str.length > 60 ? str.slice(-40) : null,
      full: str.length <= 300 ? str : undefined,
    };
  } catch (e) {
...
module.exports = { kvGet, kvSet, kvSetWithTTL, kvIncr, kvListPush, kvListGet, kvJsonUpdate, kvHashIncr, kvHashGetAll, kvDel, kvDebugRaw, kvDebugRoundtrip };
