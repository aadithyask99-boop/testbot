...
function analyseRequest(request) {
  ...
  const threshold = (config && config.detectionThreshold) || 70;
  const isBot = finalConfidence >= threshold;

  // Check if this is a Googlebot variant — never inject on these
  const cloakingRisk = uaResult.cloakingRisk || false;
  ...
  return {
    ...
    // suggestedCPM is null for cloakingRisk crawlers — no revenue from these
    suggestedCPM: (isBot && !cloakingRisk) ? getCPM(commercialValue, uaResult.crawlerType) : null,
