...
// ============================================================
// Same shuffle → stable-sort by CPM desc → walk-budgets logic
// as runAuction, but takes a pre-filtered list of campaigns.
// Used by the matching layer (api/match.js): after relevance
// filtering, the surviving campaigns are passed here for the
// final CPM-and-budget walk.
//
// IMPORTANT: caller is responsible for any prior eligibility
// filtering it cares about. This function only checks budget
// headroom (daily + total) and applies isEligible() as a safety
// net for active/in-date.
// ============================================================
async function runAuctionFromList(campaigns) {
  if (!Array.isArray(campaigns) || campaigns.length === 0) return null;
  const today = todayStr();

  // Safety net: even if caller filtered, ensure no paused/out-of-date slips in
  const eligible = campaigns.filter(c => isEligible(c, today));
  if (eligible.length === 0) return null;

  // Shuffle (fair tiebreak), then stable-sort by CPM desc
  const sorted = shuffle(eligible).sort((a, b) => (b.cpmGBP || 0) - (a.cpmGBP || 0));

  // Walk the waterfall: first campaign with budget headroom wins
...
async function recordImpression(campaign, crawlerType, pubId) {
  try {
    ...
    // Revenue in pence (new — atomic integer increments)
    const grossGBP     = (campaign.cpmGBP * ratio) / 1000;
    const pubGBP       = grossGBP * PUBLISHER_SHARE;
    const platformGBP  = grossGBP * PLATFORM_SHARE;
    const grossP       = Math.round(grossGBP    * 100);  // pence, integer
    const pubP         = Math.round(pubGBP      * 100);
    const platformP    = Math.round(platformGBP * 100);

    const revenueOps = [
      kvIncrBy('revenue:gross:total',              grossP),
      kvIncrBy(`revenue:gross:date:${today}`,      grossP),
      kvIncrBy('revenue:platform:total',           platformP),
      kvIncrBy(`revenue:platform:date:${today}`,   platformP),
    ];

    // Advertiser billing (keyed by advId if present, else campaignId)
    const advKey = campaign.advId || campaign.id;
    revenueOps.push(kvIncrBy(`revenue:advertiser:${advKey}:total`,         grossP));
    revenueOps.push(kvIncrBy(`revenue:advertiser:${advKey}:date:${today}`, grossP));

    // Publisher earnings (only if pubId known — Cloudflare Worker passes it)
    if (pubId) {
      revenueOps.push(kvIncrBy(`revenue:publisher:${pubId}:total`,         pubP));
      revenueOps.push(kvIncrBy(`revenue:publisher:${pubId}:date:${today}`, pubP));
    }
