...
module.exports = async function handler(req, res) {
  ...
  // ── GET /ad?pub=PUBLISHER_ID&cat=CATEGORY ─────────────────
  // Returns current live creative for a given category.
  // Used by publisher SDK to fetch the right creative.
  if (req.method === 'GET' && req.url.startsWith('/ad')) {
    const publisherId = (req.query && req.query.pub) || 'unknown';
    const category = (req.query && req.query.cat) || config.sponsored.category;
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Content-Type', 'application/json');
    let creative = null;
    try {
    ...
    return res.status(200).json({ publisherId, ...creative });
  }
