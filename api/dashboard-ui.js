...
'<table><thead><tr><th>Platform</th><th>Impressions</th><th>Total Visits</th><th>Unique Visits</th><th>Visit Rate</th></tr></thead>' +
...
'<tbody id="adv-queries"><tr><td colspan="3" class="empty">Queries from Perplexity and Google appear here</td></tr></tbody></table></section>' +
...
'  set("adv-campaign","<div class=\'cname\'>"+(c.advertiser||"No campaign")+"</div>"+' +
'    "<div class=\'cmeta\'>Category: "+(c.category||"—")+" &nbsp;·&nbsp; CPM: £"+(c.cpmGBP||0)+" &nbsp;·&nbsp; Updated: "+(c.updatedAt?ago(c.updatedAt):"—")+"</div>"+' +
...
'    return "<tr><td>"+p.platform+"</td><td>"+fmt(p.impressions)+"</td><td>"+tag(isTr?"training":"retrieval",isTr?"training":"retrieval")+"</td><td>"+p.ctr+"</td></tr>";' +
'  }).join("")||"<tr><td colspan=\'4\' class=\'empty\'>No visits yet</td></tr>");' +
'  var v=pubData.recentVisits||[];' +
'  set("pub-visits",v.length?v.map(function(e){' +
